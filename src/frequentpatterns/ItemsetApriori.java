...
public class ItemsetApriori{
	...
	public String getRelativeSupport(int nbObject) {
		double frequence = ((double)this.support) / ((double) nbObject);
	...
	public double getAbsoluteSupport(){
		return this.support;
	}
...
public void calculatesupport(int size){
	 double ts =0.0;
	 for(int i=0; i<size; i++){
		 List<Double> lvalue = new ArrayList<Double>();
		 for(ItemApriori item:items){
			 lvalue.add(item.getvalues().get(i));
		 }
		double min =   getMin(lvalue); 
