...
import frequentpatterns.ItemApriori;
import frequentpatterns.ItemsetApriori;
import frequentpatterns.Itemsets;


public class AlgoAgrawalFaster94 {
	private final List<RuleAgrawal> A_rules = new ArrayList<RuleAgrawal>();
	...
	public RulesAgrawal runAlgorithm(Itemsets patterns,String name) {
		this.patterns = patterns;
		rules = new RulesAgrawal("All association rules",name);
	...
	private void calculateSupport(ItemsetApriori itemset_Lk_minus_hm_P_1) {
		...
		for(ItemsetApriori itemset : patterns.getLevels().get(itemset_Lk_minus_hm_P_1.size())){
			...
			if(itemset.allTheSame(itemset_Lk_minus_hm_P_1)){
				...
				itemset_Lk_minus_hm_P_1.setSupport(itemset.getAbsoluteSupport());
				return;
			}
		}
	}
